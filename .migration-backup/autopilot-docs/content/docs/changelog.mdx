...
- **Release Alignment**: Public docs now match the current `4.0.2` package version.
- **Watcher Staging**: Nested Git repos are skipped during staging so `git add -A` does not fail on embedded repos.
- **Brand Refresh**: Public logo/banner colors now match the actual brand assets.
...
- **Free Model Defaults**:
  - OpenRouter now prefers free models first and supports custom model lists.
- **Automatic Leaderboard Sync**:
  - Watcher syncs leaderboard metrics automatically while it runs.
- **Branch Rules**:
  - Added wildcard branch policies for push blocking, commit blocking, and signing.
- **Offline Mode**:
  - Added a local-only mode for repositories that should not hit the network.
- **Commit Signing**:
  - Added signed commit support via `git commit -S`.
- **Insights Export**:
  - Added JSON export alongside CSV for reporting and automation.

### Fixed

- **Secret Scanning**: Tightened the detector so it no longer flags its own regex source.
- **Dashboard Stability**: Simplified the runtime to avoid import issues in CI and non-TTY shells.
- **Leaderboard Sync Errors**: Improved error serialization for clearer sync failures.
...
- **Intelligence (Phase 3)**:
  - **AI Commit Messages**: Integration with OpenRouter for context-aware, senior-level commit message generation.
