...
function SidebarGroup({ section, pathname, onLinkClick }: { section: NavSection; pathname: string; onLinkClick?: () => void }) {
  ...
  return (
    <div className="mb-8">
      {section.title && (
        <button onClick={() => setIsOpen(!isOpen)} className="flex items-center justify-between w-full text-left mb-3 group">
          <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-[0.2em] group-hover:text-foreground transition-colors">{section.title}</span>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                {stats?.version ? `v${stats.version}` : 'v4.0.2'}
                {stats?.downloads ? ` • ${stats.downloads.toLocaleString()} downloads` : ''}
              </p>
            </div>
          </div>
          <a
            ...
            className="flex items-center justify-center gap-2 w-full px-3 py-2 text-xs font-medium text-black bg-primary rounded-lg hover:shadow-lg hover:scale-[1.02] transition-all duration-200"
          >
