...
class Watcher {
  ...
  onFsEvent(type, filePath) {
    ...
    // Emit live event to Web Bridge
    this.bridgeClient.emitEvent('file_change', {
      type,
  ...
  scheduleProcess() {
    ...
    // Stream live countdown to Web Bridge
    const startDebounce = Date.now();
    this.debounceInterval = setInterval(() => {
      const elapsed = Date.now() - startDebounce;
      const remainingMs = Math.max(0, debounceMs - elapsed);
      const progress = Math.min(100, Math.round((elapsed / debounceMs) * 100));
      this.bridgeClient.emitEvent('debounce_tick', {
  ...
  async processChanges() {
    ...
    logger.debug('Checking git status...');
      ...
      if (commitResult.ok) {
        ...
        // Broadcast to Web Bridge
        try {
          this.bridgeClient.emitEvent('commit', {
            hash: hash || '0000000',
            message: message,
            author: 'Developer',
            branch: branch || 'develop',
            timestamp: 'just now',
            diff: changedFiles.map(f => `${f.status || 'M'} ${f.file}`).join('\n')
          }, this.repoPath);
          this.bridgeClient.emitEvent('status_update', {
            status: 'watching',
            lastCommitHash: hash,
